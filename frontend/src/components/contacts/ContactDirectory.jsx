...
function ContactModal({ kind, isOpen, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (isOpen) { setForm(emptyContact); setError(''); }
  }, [isOpen]);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      onClose();
    } catch (err) { setError(err.message || `Unable to create ${label.toLowerCase()}.`); } finally { setSaving(false); }
  };
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title={`Add ${label}`} size="lg"><form onSubmit={submit} className="space-y-5">
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <FormField label={`${label} name`} required><Input name="name" value={form.name} onChange={update} required /></FormField>
      <FormField label="Email"><Input name="email" type="email" value={form.email} onChange={update} placeholder="name@example.com" /></FormField>
      ...
      <FormField label="Credit limit"><Input name="creditLimit" type="number" min="0" value={form.creditLimit} onChange={update} placeholder="0" /></FormField>
...
export function ContactDirectory({ kind }) {
  ...
  const loadContacts = async () => {
    ...
    try {
      const response = await masterDataService.getContacts({ limit: 100 });
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col items-start justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center"><div><h2 className="mb-1 text-2xl font-bold text-navy">{title}</h2><p className="text-sm text-slate-500">Manage your active {title.toLowerCase()} and their contact details.</p></div><Button onClick={() => setModalOpen(true)}><Plus className="mr-2 h-4 w-4" />Add {singular}</Button></div>
